...
Routes = "distanceEdgeGrouped.txt"
MeasuredData = "measuredDataEdgeGrouped.txt"
pop = "population.txt"
keys = parseData.makeKeys(pop)
Change = {}

def collapseRoutes(city1, city2):
    #TODO: figure out if the "original" orig, dest of the route
    firstIsOriginal = False
    secondIsOriginal = False
    for key, collapsed in combine.items():
        if city1 in key:
            if city1 == collapsed:
                firstIsOriginal = True
            city1 = collapsed
            break
    for key, collapsed in combine.items():
        if city2 in key:
            if city2 == collapsed:
                secondIsOriginal = True
            city2 = collapsed
            break
    if city1 > city2:
        city1, city2 = city2, city1
    return city1, city2, (firstIsOriginal and secondIsOriginal)
...
with open(Routes) as f:
    ...
    for line in f:
        city1, city2, oldDist = [x.strip() for x in line.strip().split(',')]
        oldDist = float(oldDist)
        #city1, city2, placeDistance = collapseRoutes(city1, city2)
        if(city1 == city2):
            continue
        #City 1 is smaller index
        #if not placeDistance:
        #    continue
...
with open(MeasuredData) as f:
    for line in f:
        ...
        data = float(data)
        #city1, city2, trueCities = collapseRoutes(city1, city2)
        if (city1 == city2):
            continue
        if city1 in previousDistances.keys():
